...
├── store/
│   ├── toolStore.js             estado e ações das ferramentas
...
O manifest possui ícones PNG 192×192, 512×512, maskable 512×512 e Apple Touch Icon 180×180. O favicon usa `/logo.png`. Os atalhos do sistema operacional vêm do catálogo.
...
- preferência de movimento aplicada aos instrumentos sensíveis; por decisão de produto, a composição global e a suavização Lenis/GSAP permanecem ativas;
- contraste reforçado e forced colors;
...
O GitHub é a fonte oficial do projeto. O checkout local serve para testes e
edição antes do envio; alterações locais sem commit não representam a versão
publicada. Antes de começar, execute `git fetch origin` e confira a relação
entre `HEAD` e `origin/main`, preservando eventuais rascunhos locais.
...
Após o envio, confirme o mesmo SHA em `HEAD` e `origin/main`, ausência de
diferenças em `git diff origin/main` e checkout limpo. Dependências, builds e
relatórios ignorados pelo Git são artefatos locais, não conteúdo do repositório.
Conclua a publicação somente após o workflow de deploy passar e a execução de
`npm run check:live` verificar o domínio publicado.

...
export function useAppKeyboard(toggleFullscreen, restoreInterface) {
  const { activateMode } = useToolStore();
  ...
  useKeyboardShortcuts({
    ...
    handlers: {
      ...
      openDisplayInfo: () => { activateMode(MODES.DISPLAY_INFO); showToast('I', 'Ficha Técnica'); },
      openRefreshRate: () => { activateMode(MODES.REFRESH_RATE); showToast('R', 'Medidor de Hz'); },
      openReactionTest: () => { activateMode(MODES.REACTION_TEST); showToast('A', 'Tempo de Reação'); },
      openUniformity: () => { activateMode(MODES.UNIFORMITY); showToast('U', 'Uniformidade'); },
      openGradient: () => { activateMode(MODES.GRADIENT_BANDING); showToast('D', 'Gradiente e Banding'); },
      openMouseTest: () => { activateMode(MODES.MOUSE_TEST); showToast('O', 'Teste de Mouse'); },
      restoreInterface,
  ...
  useEffect(() => {
    ...
    const handleGlobalKeyDown = (e) => {
      ...
      const isEditableTarget = e.target instanceof Element && Boolean(e.target.isContentEditable || e.target.closest('input, textarea, select, [contenteditable]'));

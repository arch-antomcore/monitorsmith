...
export function useWakeLock({ autoRequest = false } = {}) {
  ...
  const requestWakeLock = useCallback(async () => {
    ...
    if (canUseDocument() && document.visibilityState === 'hidden') return false;
    ...
    const requestPromise = (async () => {
      try {
        ...
        if (!mountedRef.current || !shouldRestoreRef.current || generationRef.current !== requestGeneration) {
  ...
  const releaseWakeLock = useCallback(async () => {
    shouldRestoreRef.current = false;
    generationRef.current += 1;
    const releaseGeneration = generationRef.current;
    requestInFlightRef.current = null;
    ...
    try {
      ...
      if (mountedRef.current && generationRef.current === releaseGeneration) {
        if (!sentinel.released && !sentinelRef.current) {
          sentinelRef.current = sentinel;
          setIsLocked(true);
        }
        setError(
  ...
  const toggleWakeLock = useCallback(() => {
    const activeSentinel = sentinelRef.current;
    if ((activeSentinel && !activeSentinel.released) || requestInFlightRef.current) {

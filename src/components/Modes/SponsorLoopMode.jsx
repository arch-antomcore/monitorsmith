...
export default function SponsorLoopMode({
  ...
  useEffect(() => {
    ...
    if (el?.matches?.('button, a[href], input, textarea, select, [tabindex], [contenteditable]')) return;
  ...
  const persistImages = (next) => {
    ...
    void saveSponsorImages(next).then((result) => {
      if (!isMountedRef.current || revision !== persistenceRevisionRef.current) return;
      if (!result.success) {
        setUploadMessage((message) => `${message} (Aviso: Não foi possível salvar no banco local).`);
      }
    });
  };

  const processFiles = async (fileList, generation) => {
    const isCurrent = () => isMountedRef.current && generation === importGenerationRef.current;
    if (!isCurrent()) return;
  ...
  const enqueueFiles = (fileList) => {
    ...
    importQueueRef.current = importQueueRef.current
      .catch(() => undefined)
      .then(() => processFiles(files, generation));
  ...
  useEffect(() => {
    ...
    const generation = importGenerationRef.current;
    // Restore before any new imports so a delayed database read cannot append
    // old files to a replacement playlist or overwrite a newly saved import.
    importQueueRef.current = importQueueRef.current.catch(() => undefined).then(async () => {
      if (!active) return;
      const res = await loadSponsorImages();
      if (!active || generation !== importGenerationRef.current) return;
      const files = (res.data || []).map((saved) => saved.file).filter(Boolean);
      if (files.length) await processFiles(files, generation);
    });
    return () => { active = false; };
  ...
  const clearAll = () => {
    importGenerationRef.current += 1;
    // Decoding cannot be aborted, but its result and queued older imports must
    // no longer be allowed to repopulate the explicitly cleared playlist.
    importQueueRef.current = Promise.resolve();
    const revision = ++persistenceRevisionRef.current;
    images.forEach((image) => URL.revokeObjectURL(image.src));
    setImages([]);
    imagesRef.current = [];
    void clearSponsorImages().then((result) => {
      if (!isMountedRef.current || revision !== persistenceRevisionRef.current) return;
      setUploadMessage(result.success
        ? 'Todas as imagens foram removidas do armazenamento local.'
        : 'Não foi possível remover as imagens do armazenamento local. Elas podem reaparecer ao reabrir a ferramenta.');
    });
    setActiveIndex(0);
    setIsPlaying(false);
    playbackDeadlineRef.current = 0;
    updateProgress(0);
    setUploadMessage('Removendo imagens do armazenamento local...');
  };

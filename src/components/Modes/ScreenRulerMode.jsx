...
export default function ScreenRulerMode({ onExit }) {
  ...
  // Retain real units while omitting subdivisions too dense to distinguish.
  const tickStep = 10 ** Math.max(0, Math.ceil(Math.log10(3 / (metrics.pxPerUnit / 10))));
  const tickCount = Math.floor(visibleLength * 10 / tickStep) + 1;
                ...
                return (
                  <span
                    ...
                    style={{
                      position: 'absolute',
                      bottom: 0,
                      left: `${(tick / 10) * metrics.pxPerUnit}px`,
                      width: 1,

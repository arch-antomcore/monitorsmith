...
export default React.memo(function AdaptiveNavbar({
  ...
  const [homeExpanded, setExpanded] = useState(true);
  const isExpanded = showModeContext || homeExpanded;
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    if (showModeContext || document.activeElement?.closest('.wbp-navbar')) return;
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      url: `https://monitorsmith.app/${window.location.search}${window.location.hash}`,
    };
    if (navigator.share) {
      try {
    ...
    try {
      ...
      showToast('Link', 'Não foi possível copiar. Copie o endereço da barra do navegador.');
    }
  };
  ...
  return (
    ...
    >
      <motion.nav
        ...
        // Keep width outside Motion so an interrupted collapse cannot clip tool controls.
        style={{ width: isExpanded ? 'max-content' : '3rem', willChange: 'transform' }}
        whileHover={!isExpanded && !shouldReduceMotion ? { scale: 1.03 } : {}}
        whileTap={!isExpanded && !shouldReduceMotion ? { scale: 0.95 } : {}}
        aria-hidden={!visible}
        inert={!visible ? '' : undefined}
        aria-label="Controles do MonitorSmith"
        className={cn(
      ...
      >
        <motion.div
          ...
          transition={shouldReduceMotion ? { duration: 0.001 } : undefined}
        ...
        >
          {showModeContext && onBrandClick ? (
            <button 
              ...
              className="flex items-center gap-2 hover:opacity-80 transition-opacity outline-none min-h-[32px] py-1"
        ...
        >
          {showModeContext && onBrandClick && (
            <motion.button whileHover={!shouldReduceMotion ? { scale: 1.15, transition: { type: "spring", stiffness: 400, damping: 17 } } : undefined} whileTap={!shouldReduceMotion ? { scale: 0.95 } : undefined} transition={shouldReduceMotion ? { duration: 0.001 } : undefined} variants={itemVariants} onClick={onBrandClick} aria-label="Voltar às ferramentas" title="Voltar às ferramentas (H)" className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors px-2 py-1 flex items-center justify-center">
              <ControlIcon name="home" size={20} />
            </motion.button>
          )}

          {onToggleFullscreen && (
            <motion.button whileHover={!shouldReduceMotion ? { scale: 1.15, transition: { type: "spring", stiffness: 400, damping: 17 } } : undefined} whileTap={!shouldReduceMotion ? { scale: 0.95 } : undefined} transition={shouldReduceMotion ? { duration: 0.001 } : undefined} variants={itemVariants} onClick={onToggleFullscreen} aria-label={fullscreenLabel} title={fullscreenLabel} className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors px-2 py-1 flex items-center justify-center">
          ...
          {showModeContext && onHideUi && (
            <motion.button whileHover={!shouldReduceMotion ? { scale: 1.15, transition: { type: "spring", stiffness: 400, damping: 17 } } : undefined} whileTap={!shouldReduceMotion ? { scale: 0.95 } : undefined} transition={shouldReduceMotion ? { duration: 0.001 } : undefined} variants={itemVariants} onClick={onHideUi} aria-label="Ocultar barras e interface (Modo imersivo)" title="Ocultar barras e controles (×)" className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors px-2 py-1 flex items-center justify-center">
              <ControlIcon name="x" size={20} />
            </motion.button>
          )}

          {onOpenHelp && (
            <motion.button whileHover={!shouldReduceMotion ? { scale: 1.15, transition: { type: "spring", stiffness: 400, damping: 17 } } : undefined} whileTap={!shouldReduceMotion ? { scale: 0.95 } : undefined} transition={shouldReduceMotion ? { duration: 0.001 } : undefined} variants={itemVariants} onClick={onOpenHelp} aria-label="Abrir atalhos de teclado" title="Atalhos de teclado (? / K)" className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors px-2 py-1 flex items-center justify-center">
        ...
        {!isExpanded && (
          <button
            type="button"
            aria-label="Expandir controles do MonitorSmith"
            aria-expanded="false"

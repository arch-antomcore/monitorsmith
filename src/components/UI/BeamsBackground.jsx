...
function createBeam(width, height, isDarkMode) {
  ...
  // Adjusted hue to match MonitorSmith's "Arctic Glass" / Tech aesthetic (cyan/blue/emerald)
  const hueBase = isDarkMode ? 170 : 190;
  const hueRange = 40;
...
export default function BeamsBackground({
  ...
  useEffect(() => {
    ...
    function resetBeam(beam, index, totalBeams) {
      ...
      const hueBase = isDarkModeRef.current ? 170 : 190;
      const hueRange = 40;
  ...
  return (
    <motion.div
      className={cn(
        "pointer-events-none absolute inset-0 w-full overflow-hidden mix-blend-screen",
        className
      )}
      style={{ ...style, height: "150vh", top: "-25vh" }}
    >
      <canvas

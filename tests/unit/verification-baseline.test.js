...
describe('contratos estáticos complementares do produto', () => {
  ...
  it('não limita animações por preferência de movimento do sistema na composição', async () => {
    const appJsx = await readFile(path.join(root, 'src', 'App.jsx'), 'utf8')
    expect(appJsx).toContain('reducedMotion="never"')
    expect(appJsx).not.toContain('reducedMotion="user"')
  })

  it('mantém GSAP local e as integrações de Lenis e Motion', async () => {
    const [gsapCore, viteConfig, appJsx, toolExplorer] = await Promise.all([
    ...
    expect(appJsx).not.toContain("from 'gsap/ScrollTrigger'")
    expect(appJsx).not.toContain("from 'gsap/CustomEase'")
    expect(appJsx).not.toContain('gsap.registerPlugin')
    expect(appJsx).not.toContain('ScrollTriggerSync')
    expect(appJsx).not.toContain('useLenis')
    expect(appJsx).toContain('gsap.ticker.add(update)')
    expect(appJsx).toContain('gsap.ticker.remove(update)')
    expect(appJsx).toContain('autoRaf={false}')
    expect(appJsx).toContain('smoothWheel: true')
  ...
  it('preserva os contratos técnicos de AdSense, consentimento e páginas legais', async () => {
    ...
    expect(consentLib).toContain('export const ADSENSE_SCRIPT_SRC = `https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=${ADSENSE_CLIENT}`')
    expect(consentLib).toContain("script.crossOrigin = 'anonymous'")
    ...
    expect(seoScript).toContain("sc.crossOrigin='anonymous'")
  })
})

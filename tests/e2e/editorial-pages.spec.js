...
test('exemplo publicado de PPI pode ser reproduzido com os dois presets de 27 polegadas', async ({ page }) => {
  await page.goto('/calculadora-ppi-densidade-monitor/')
  await expect(page.getByText('Exemplo para reproduzir na calculadora:', { exact: false })).toContainText('108,79 PPI')
  await page.goto('/#ppi-calculator')
  await expect(page.getByText('108.79', { exact: true })).toBeVisible()
  await expect(page.getByText('0.2335', { exact: true })).toBeVisible()
  await expect(page.getByText('80', { exact: true })).toBeVisible()
  await page.getByRole('button', { name: '27" 4K UHD', exact: true }).click()
  await expect(page.getByText('163.18', { exact: true })).toBeVisible()
  await expect(page.getByText('0.1557', { exact: true })).toBeVisible()
  await expect(page.getByText('54', { exact: true })).toBeVisible()
})

test('calculadora PPI mantém presets clicáveis e resultados roláveis em tela baixa', async ({ page }) => {
  await page.setViewportSize({ width: 844, height: 390 })
  await page.emulateMedia({ reducedMotion: 'no-preference' })
  await page.goto('/#ppi-calculator')
  const preset = page.getByRole('button', { name: '27" 4K UHD', exact: true })
  await preset.click()
  await expect(page.getByText('163.18', { exact: true })).toBeVisible()
  await preset.hover()
  await page.mouse.wheel(0, 650)
  await expect.poll(() => page.locator('.display-mode--ppi-calculator').evaluate((element) => element.scrollTop)).toBeGreaterThan(100)
  await page.getByRole('button', { name: 'Copiar Relatório Técnico de Densidade' }).click()
  await expect(page.getByText('54 cm', { exact: true })).toBeVisible()
})

...
async function expectScrollablePpiPanel(page, geometry) {
  // Instructions follow the calculator results in the document. Native
  // scrolling and real preset clicks are covered in editorial-pages.spec.js.
  await expect(page.locator('.display-mode--ppi-calculator')).toHaveCSS('overflow-y', 'auto')
  await expect(page.locator('.display-mode__controls')).toHaveCSS('position', 'relative')
...
test('home apresenta o catálogo completo sem violações automáticas WCAG A/AA', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Abrir a bancada' })).toBeVisible()
...
for (const toolId of TOOL_IDS) {
  test(`abre e fecha a ferramenta ${toolId} por interação real`, async ({ page }) => {
    ...
    if (geometry.panel && toolId === 'ppi-calculator') {
      await expectScrollablePpiPanel(page, geometry)
    } else if (geometry.panel) {
...
test('menu radial é nomeado e realmente troca a ferramenta', async ({ page }) => {
  ...
  await expect(menu.getByRole('menuitem')).toHaveCount(26)
...
test('swipe troca o canvas, mas nunca dispara a partir dos controles', async ({ page }) => {
  await page.goto('/#dead-pixel')
  await page.waitForSelector('.display-mode__controls')

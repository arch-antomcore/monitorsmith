...
test('sem slots, a home não pede consentimento antecipado e mantém preferências no rodapé', async ({ page }) => {
  const adRequests = [];
  ...
  // Covers the former 1200 ms automatic banner, rather than asserting before it opens.
  await page.waitForTimeout(1500);
  await expect(page.locator('[data-consent-banner]')).toHaveCount(0);
  await page.getByTestId('footer-consent').click();
  await expect(page.getByRole('dialog', { name: 'Privacidade, cookies e anúncios' })).toBeVisible();
  await page.getByTestId('consent-toggle-ads').check();

...
O arquivo `src/constants/tools.js` define as 27 ferramentas. Nenhum componente deve manter outra lista independente.
...
A versão inglesa pode explicar uma ferramenta para busca internacional. A home, o catálogo e os instrumentos novos oferecem português, inglês e espanhol, mas parte dos controles originais permanece em português. Não se deve declarar cobertura integral de tradução enquanto essa diferença existir, nem declarar uma “home inglesa” apontando para um guia isolado.
...
- guia en: `0.7`;
- artigo: `0.7`;
- página institucional ou legal: `0.5`.
...
Os 44 guias usam `TechArticle`, `HowTo`, `WebApplication`, `FAQPage` e `BreadcrumbList`. Esses dados descrevem cada rota editorial e precisam corresponder ao conteúdo visível; não provam capacidade de medição física nem aprovação de rich results. Páginas institucionais e legais usam `WebPage`.
...
O service worker é gerado a partir do artefato final. O núcleo do app, os chunks necessários, fontes e ícones entram no precache; páginas editoriais e imagens são armazenadas sob demanda. AdSense, DoubleClick, Google consent e `ads.txt` não podem ser armazenados no cache do app.
...
As fontes são locais por pacotes `@fontsource`; não existe dependência do Google Fonts.
...
Instrumentos sensíveis a movimento consultam `prefers-reduced-motion` para iniciar em pausa ou reduzir efeitos próprios. Por decisão de produto, a composição global e a suavização Lenis/GSAP permanecem ativas; documentação e testes devem distinguir essas duas camadas. Testes automatizados com axe complementam, mas não substituem teclado, zoom de 200%/400% e leitor de tela.

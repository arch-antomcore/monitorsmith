...
## 1. Convenção experimental `llms.txt`

O arquivo `/llms.txt` é uma proposta comunitária para apresentar propósito, rotas e conteúdo de um site em texto simples. A adoção varia entre sistemas, e sua presença não garante descoberta, indexação ou citação por modelos de linguagem.

### Estrutura proposta para `llms.txt`:
...
- As ferramentas indicadas processam os dados descritos localmente; hospedagem e integrações externas podem gerar requisições de rede
...
## 3. Referência opcional via HTML (`rel="describedby"`)

É possível incluir a tag abaixo no `<head>` como uma referência ao arquivo. Ela não é um mecanismo universal de descoberta e deve ser tratada como metadado experimental:
...
Projetos podem fornecer dados semânticos estruturados via JSON-LD quando o tipo escolhido corresponde ao conteúdo visível da página.
...
Para tornar o conteúdo rastreável sem depender da execução do JavaScript, sem prometer indexação ou aprovação no AdSense:
...
3. **Benefícios:**
   - Crawlers sem JS podem ler o HTML, títulos e metadados entregues pelo servidor.
   - Navegadores humanos carregam a versão estática e hidratam o app interativo.
   - Compatibilidade com hospedagem estática, quando o projeto não depende de processamento no servidor.

...
if (checkAdsTxt(adsTxt).length) throw new Error(checkAdsTxt(adsTxt).join('; '));
...
for (const file of htmlFiles) {
  ...
  const adProblems = checkAdSenseHtml(html, { allowAds: !isLegacyRedirect && rel !== '404.html' });
  for (const problem of adProblems) console.error(`  ❌ AdSense em ${rel}: ${problem}`);
  adSenseErrors += adProblems.length;
...
console.log(`- Falhas na integração pública AdSense: ${adSenseErrors}`);
...
if (brokenLinks > 0 || brokenImages > 0 || adSenseErrors > 0 || jsonLdErrors > 0 || missingCanonical > 0 || sitemapMissing > 0 || legacyRedirects !== 14) {
  console.log('\n❌ AUDITORIA REPROVADA.');
  process.exit(1);
} else {
  console.log(`\nAuditoria de integridade concluída: ${htmlFiles.length} arquivos HTML e ${locs.length} URLs do sitemap verificados.`);
}

...
import { checkAdSenseHtml, checkAdsTxt } from './adsense-checks.mjs'
...
const canonicalResults = await mapConcurrent(sitemapUrls, 8, async (url) => {
  try {
    ...
    errors.push(...checkAdSenseHtml(text).map((error) => `${url}: ${error}`))
    if (/noindex|none/i.test(response.headers.get('x-robots-tag') || '')) errors.push(`${url} bloqueada por X-Robots-Tag`)
...
const migrationResults = await mapConcurrent(LEGACY_REDIRECTS, 6, async ([sourcePath, targetPath]) => {
  ...
  try {
    ...
    errors.push(...checkAdSenseHtml(text, { allowAds: false }).map((error) => `${sourcePath}: ${error}`))
...
for (const asset of ['/ads.txt', '/robots.txt', '/llms.txt', '/llms-full.txt', '/manifest.webmanifest', '/sw.js']) {
  try {
    ...
    if (asset === '/ads.txt') errors.push(...checkAdsTxt(text))
...
try {
  ...
  errors.push(...checkAdSenseHtml(text, { allowAds: false }).map((error) => `404: ${error}`))
...
// Public Search Console files prove only that the expected token is reachable.
// Account ownership and the verification status must be checked in the account.
const verificationFiles = (await readdir(new URL('../public/', import.meta.url)))
  .filter((name) => /^google[a-z0-9]+\.html$/i.test(name))
for (const name of verificationFiles) {
  try {
    const expected = (await readFile(new URL(`../public/${name}`, import.meta.url), 'utf8')).trim()
    const { response, text } = await fetchText(`${BASE_URL}/${name}`, { redirect: 'manual' })
    if (response.status !== 200 || text.trim() !== expected) errors.push(`${name}: arquivo de verificação diferente ou indisponível`)
  } catch (error) { errors.push(`${name} falhou: ${error.message}`) }
}
...
console.log(`Identificação: meta de publisher nas ${sitemapUrls.length} canônicas, ads.txt completo e ${verificationFiles.length} arquivo(s) público(s) do Search Console conferidos.`)

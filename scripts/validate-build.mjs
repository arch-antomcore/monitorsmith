...
for (const file of htmlFiles) {
  ...
  errors.push(...checkAdSenseHtml(html, { allowAds: !isLegacyRedirect }).map((error) => `${relative}: ${error}`))
...
errors.push(...checkAdSenseHtml(notFound, { allowAds: false }).map((error) => `404.html: ${error}`))

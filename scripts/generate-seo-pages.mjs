...
import { BLOG_READING_MINUTES, countWords, stripHtml } from './blog-reading-times.mjs';
...
(function(){
  ...
  function read(){if(sessionConsent)return sessionConsent;try{var raw=JSON.parse(localStorage.getItem(KEY));if(raw&&raw.decided===true&&typeof raw.ads==='boolean'&&typeof raw.personalization==='boolean')return sessionConsent={decided:true,ads:raw.ads,personalization:raw.ads&&raw.personalization};}catch(e){}return null}
  function update(state){
    if(window.adsbygoogle)window.adsbygoogle.requestNonPersonalizedAds=state.personalization?0:1;
    if(!state.ads&&pendingScript){pendingScript.onload=null;pendingScript.onerror=null;pendingScript.remove();pendingScript=null;}
    try{gtag('consent','update',{ad_storage:state.ads?'granted':'denied',ad_user_data:state.personalization?'granted':'denied',ad_personalization:state.personalization?'granted':'denied',analytics_storage:'denied'});}catch(e){}
  }
  function loadAds(state){
    ...
    pendingScript=sc;
    sc.onerror=function(){sc.remove();if(pendingScript===sc)pendingScript=null;};
    sc.onload=function(){
      if(pendingScript!==sc)return;
      pendingScript=null;
      if(!read()||!read().ads){sc.remove();return;}
  ...
  function decide(ads,personalization){
    ...
    try{localStorage.setItem(KEY,JSON.stringify(state));persisted=true;}catch(e){}
    update(state);
    if(persisted&&previous&&previous.ads&&(previous.ads!==state.ads||previous.personalization!==state.personalization)){window.location.reload();return}
    loadAds(state);
  ...
  window.addEventListener('storage',function(event){
    ...
    sessionConsent=null;
    var next=read();
    var state=next||{decided:false,ads:false,personalization:false};
    update(state);
    ...
    var banner=document.getElementById('ms-consent');
    if(banner){banner.remove();openPreferences();}
    else if(!next&&hasAdSlots)openPreferences();
    loadAds(state);
  });
})();
</script>`;
...
const EDITORIAL_CONTENT = Object.freeze({
  ...
  'monitor-test': {
    ...
    pt: {
      ...
      faq: [
        ['Qual a diferença entre o Teste de Monitor e a Verificação Visual?', 'Os dois guias abrem o mesmo instrumento de padrões. Este guia orienta uma triagem de recebimento; o guia de Verificação Visual explica a comparação de tons, gradientes e nitidez antes e depois de um ajuste.'],
    ...
    en: {
      ...
      steps: ['Open the test fullscreen immediately after unboxing or before completing a purchase.', 'Go through the visual checklist: geometric grid alignment, basic uniformity, and absence of artifacts.', 'Record any repeatable inconsistency and check the applicable warranty or return policy.'],
      ...
      faq: [
        ['What is the difference between Monitor Test and Visual Check?', 'Both guides open the same pattern tool. This guide covers an initial receiving inspection; the Visual Check guide explains how to compare tones, gradients and sharpness before and after an adjustment.'],
  ...
  'display-calibration': {
    ...
    pt: {
      ...
      steps: ['Anote o modo de imagem, o brilho, o perfil de cor e a escala atuais; estabilize a iluminação da sala antes de alterar qualquer ajuste.', 'Selecione Escala de cinza para comparar os 17 degraus e Uniformidade 5% para observar os cinco campos escuros entre 0% e 10%.', 'Compare Gradiente CSS, Grade de nitidez e Subpixel RGB/BGR. Registre faixas, halos ou franjas aparentes; esses padrões não identificam FRC nem a profundidade física de bits.'],
    ...
    en: {
      ...
      steps: ['Record the current picture mode, brightness, colour profile and scaling; stabilize room lighting before changing any setting.', 'Select the grayscale pattern to compare 17 steps, then the near-black pattern labelled Uniformidade 5% to inspect five dark fields from 0% to 10%.', 'Compare the CSS gradient, sharpness grid and RGB/BGR text patterns. Record visible bands, halos or fringes; these patterns cannot identify FRC or physical bit depth.'],
  ...
  'webcam-light': {
    ...
    pt: {
      intro: 'Utiliza a tela do monitor como fonte de luz de apoio para videochamadas e gravações, com intensidade ajustável e uma escala nominal de cor em kelvin.',
      steps: ['Posicione a janela do navegador em frente ao seu rosto ou em um segundo monitor.', 'Ajuste a intensidade e a escala nominal de 1800 K a 12000 K; compare o resultado na prévia da câmera com a iluminação da sala.', 'Reduza o brilho físico se houver reflexos indesejados em óculos.'],
      ...
      limitations: 'O valor em kelvin escolhe uma aproximação RGB; não mede a temperatura de cor emitida. A luz que chega ao assunto depende do brilho, tamanho, distância e ângulo da tela, além do comportamento do painel.',
      faq: [
        ...
        ['O que significa o controle em kelvin?', 'É uma referência nominal para escolher uma aparência mais quente ou mais fria. O navegador converte o valor em RGB; selecionar 6500 K não calibra o monitor para D65 nem reproduz o espectro da luz do dia.']
      ],
      methodology: [
    ...
    en: {
      intro: 'Uses your display as a fill-light source for video calls and recordings, with adjustable intensity and a nominal colour scale in kelvin.',
      steps: ['Position the browser window facing your subject or onto a secondary display.', 'Adjust intensity and the nominal scale from 1800 K to 12000 K; compare the result in the camera preview with the room lighting.', 'Lower screen luminance if specular reflections appear on eyeglasses.'],
      ...
      limitations: 'The kelvin value selects an RGB approximation; it does not measure emitted colour temperature. Light reaching the subject depends on screen brightness, size, distance and angle, as well as panel behaviour.',
      faq: [
        ...
        ['What does the kelvin control mean?', 'It is a nominal reference for choosing a warmer or cooler appearance. The browser converts the value to RGB; selecting 6500 K neither calibrates the monitor to D65 nor reproduces the spectrum of daylight.']
      ],
      methodology: [
  ...
  'sponsor-loop': {
    ...
    pt: {
      ...
      steps: ['Selecione as imagens que possui autorização para exibir.', 'Configure a ordem, o tempo por slide de 1 a 60 segundos, o tipo de transição e a cor de fundo.', 'Inicie a sequência e solicite tela cheia pelo botão ou pela tecla F.'],
    ...
    en: {
      ...
      steps: ['Select the brand images you are authorized to display.', 'Configure sequence order, per-slide duration from 1 to 60 seconds, transition style, and background color.', 'Start playback and request fullscreen with the button or the F key.'],
  ...
  'ppi-calculator': {
    ...
    pt: {
      ...
      methodology: [
        ...
        'Exemplo para reproduzir na calculadora: selecione 27" 1440p (2560 × 1440). O resultado é 108,79 PPI, dot pitch de 0,2335 mm e referência angular de 80 cm. Mantendo 27" e mudando para 3840 × 2160, os valores passam a 163,18 PPI, 0,1557 mm e 54 cm. São resultados calculados das entradas, não medições de dois monitores físicos.',
    ...
    en: {
      ...
      methodology: [
        ...
        'Reproduce this example in the calculator: select 27" 1440p (2560 × 1440). The result is 108.79 PPI, a 0.2335 mm dot pitch and an 80 cm angular reference. Keep the 27" diagonal and switch to 3840 × 2160: the results become 163.18 PPI, 0.1557 mm and 54 cm. These values are calculated from the inputs, not measurements of two physical monitors.',
  ...
  },
  {
    ...
    sections: [
      ['1. Compromisso', [
        ...
        'A navegação principal e os controles centrais são operáveis por teclado e recebem rótulos programáticos durante o desenvolvimento e a revisão. A preferência prefers-reduced-motion pausa instrumentos sensíveis; por decisão de produto, as transições estruturais e a suavização de rolagem permanecem ativas.',
      ]],
      ['2. Recursos implementados', [
        ...
        '• Instrumentos sensíveis iniciados em pausa quando o sistema sinaliza preferência por menos movimento; as transições estruturais e a suavização de rolagem permanecem ativas.',
...
function renderBlogArticle(article, illustratedBody) {
  ...
  const readingMinutes = BLOG_READING_MINUTES[article.slug];

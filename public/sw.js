...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith((async () => {
    const shell = await caches.open(CACHE_VERSION)
    // The same-origin precache contains fixed build files. Module requests may
    // carry Origin while installation requests do not; preview/CDN Vary headers
    // must not turn an already cached build asset into an offline network miss.
    const shellAsset = await shell.match(event.request, { ignoreVary: true })
